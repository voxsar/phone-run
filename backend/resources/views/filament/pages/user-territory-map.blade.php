<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <x-filament::section>
            <div class="flex items-center gap-4">
                <div class="w-8 h-8 rounded-full border-2 border-white" style="background-color: {{ $user->color ?? '#3388FF' }}"></div>
                <div>
                    <p class="font-semibold text-gray-900 dark:text-white">{{ $user->name }}</p>
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ $user->email }}</p>
                </div>
                <div class="ml-auto text-right">
                    <p class="text-2xl font-bold text-amber-500">{{ count(json_decode($territoriesJson, true) ?? []) }}</p>
                    <p class="text-xs text-gray-500">Active territories</p>
                </div>
            </div>
        </x-filament::section>
        ...
        {{-- ── Google Map ───────────────────────────────────────────── --}}
        <x-filament::section heading="{{ $user->name }}'s Territory Map">
            <div id="user-territory-map" style="height:600px;width:100%;border-radius:0.5rem;overflow:hidden;"></div>
        </x-filament::section>
    ...
    <script>
        ...
        function initMap() {
            map = new google.maps.Map(document.getElementById('user-territory-map'), {
                zoom:      3,
                center:    { lat: 20, lng: 0 },
                mapTypeId: 'roadmap',
            });
